...
path=".\dati\curva_forte\";
% path=".\dati\curva_piano\";
sr=25; % sample rate

rilievo=2; % 2-4
% rilievo=7; % il 7 prende la curva in direzione opposta alle altre misure
...
for f=1:length(fun)
    ...
    % sezione(4,:)=[28 t(end)];
    % 
    % str=["Accelerate","Tourning","Accelerate","Brake"];
    % 
    % L=length(funzione);
...
% exportgraphics(f,"..\slide\curva\figure\"+fun_str+"\"+tit+".png")
...
% exportgraphics(f,"..\slide\curva\figure\"+fun_str+"\Trasformata\"+tit+".png")
...
for j=1:length(fun_axes)
    ...
    % exportgraphics(f,"..\slide\curva\figure\"+fun_str+"\Trasformata\"+tit+fun_axes(j)+".png")
end
